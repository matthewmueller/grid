...
Selection.prototype.forEach = function(fn) {
  ...
  for (var i = 0, at; at = sel[i]; i++) {
    cell = grid[at] = grid[at] || new Cell;
    var ret = fn(cell.value(), at, i);
    if (undefined !== ret) cell.value(ret);
    if (false === ret) break;
  }
...
Selection.prototype.shiftrows = function(s) {
  ...
  for (var i = 0, at; at = expanded[i]; i++) {
    ...
    var shifted = m[1] + n;
    grid[shifted].value(grid[at].value());
    grid[at].value(null);
  }
...
Selection.prototype.find = function(expr) {
  ...
  // hack to get the types working correctly
